...
func (h *RecipeHandler) GetRecipes(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.DB.Query("SELECT id, name, instructions FROM recipes")
	if err != nil {
	...
	for rows.Next() {
		var r models.Recipe
		if err := rows.Scan(&r.ID, &r.Name, &r.Instructions); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func (h *RecipeHandler) AddRecipeIngredient(w http.ResponseWriter, r *http.Request) {
	...
	if req.IngredientID == 0 && req.IngredientName != "" {
		...
		if err != nil {
			err = h.DB.QueryRow(
				"INSERT INTO ingredients (name, current_stock, price) VALUES ($1, 0, NULL) RETURNING id",
				req.IngredientName,
			).Scan(&req.IngredientID)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
